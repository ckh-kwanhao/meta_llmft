...
def distillation_loss(student_logits, teacher_logits, labels, temperature=2.0, alpha=0.5):
    """
    Compute the knowledge distillation loss between the teacher and student models.
    ...
    :param temperature: Temperature parameter to soften the probability distributions
    :param alpha: Weight for the distillation loss vs. the standard classification loss
    :return: Combined knowledge distillation loss
    """
    # Soften the logits by dividing by the temperature
    student_soft_logits = functional.log_softmax(student_logits / temperature, dim=-1)
    teacher_soft_logits = functional.softmax(teacher_logits / temperature, dim=-1)
    
    # Calculate the KL divergence loss (distillation loss)
    distillation_loss = functional.kl_div(student_soft_logits, teacher_soft_logits, reduction='batchmean') * (temperature ** 2)
    
    # Calculate the standard cross entropy loss (classification loss)
    classification_loss = functional.cross_entropy(student_logits, labels)
...
class OPTWithLMClassifierKD(OPTWithLMClassifier):
    def __init__(self, config):
        super().__init__(config)
    ...
    def from_pretrained(
        ...
        if teacher_model:
            ...
            for param in cls.teacher_model.parameters():
                param.requires_grad = False
            cls.teacher_model.eval()
        else:
            raise NotImplementedError(
                f"teacher_model is None")
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss = distillation_loss(logits, teacher_logits, labels, temperature=2.0, alpha=0.5)
...
# from torch.nn.functional import log_softmax, softmax, kl_div, cross_entropy
