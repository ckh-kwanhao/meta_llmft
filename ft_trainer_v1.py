...
class FtTrainer(Trainer):
    ...
    def __init__(
        ...
        if not eval_only:
            ...
            if (
                ft_args.deepspeed_stage != 3
            ):  # with stage 3 the weights are partitioned across GPUs, this makes logging weights complicated
            ...
            if self.model_starting_params is not None:
                for n, p in model.module.named_parameters():
                    if n in self.trainable_params_names:
                        ...
                        l2_dist = torch.dist(torch.flatten(sp), torch.flatten(p), p=2)
...
class KdTrainer(FtTrainer):
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            ...
            if isinstance(teacher_outputs, dict):
                teacher_logits = tuple(
                    v for k, v in teacher_outputs.items() if k not in ignore_keys
                )
            else:
            ...
            if has_labels or loss_without_labels:
                ...
                for student_logit, teacher_logit in zip(student_logits, teacher_logits):
                    loss += kl_div(log_softmax(student_logit, dim=-1), softmax(teacher_logit, dim=-1), reduction='batchmean')
                    print(f"loss: {loss}")
                loss /= len(student_logits)
                print(f"Total loss: {loss}")
                # loss = kl_div(log_softmax(student_logits, dim=-1), softmax(teacher_logits, dim=-1), reduction='batchmean')
        ...
        ## end modification for KL divergence

        if prediction_loss_only:
            return (loss, None, None)

        student_logits = nested_detach(student_logits)
        if len(student_logits) == 1:
