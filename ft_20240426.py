...
def main():
    ...
    if training_args.do_train:
        ...
        # Save model
        print("Saving model")
        trainer.save_model("/llmft/model/teacher_model")

        # had issue saving model - changed deepspeed/ds_config_zero3.json 
            #     "zero_optimization": {
            # "stage": 3,
            # "stage3_gather_16bit_weights_on_model_save": true, from false to true
            # "reduce_scatter": true,
            # "overlap_comm": true,
        ...
        if trainer.is_world_process_zero():
            tokenizer.save_pretrained("/llmft/model/teacher_model/tokens")
        ...
        if ft_args.target_tokens is not None:
            ...
            t_model = OPTWithClassifier.from_pretrained(
                "/llmft/model/teacher_model"    ,
                from_tf=bool(".ckpt" in model_args.model_name_or_path),
                config=config,
                cache_dir=model_args.cache_dir,
        ...
        print("Loaded saved model into trainer.")
        ...
        train_result = s_trainer.train(resume_from_checkpoint=checkpoint, ignore_keys_for_eval=["past_key_values"])
