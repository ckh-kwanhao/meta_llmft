...
from ft_trainer import FtTrainer, KdTrainer
...
def main():
    ...
    ## 20240426 kwanhao_chan config definition which receives arg 
    ...
    ### Start block -- get kd training dataset ###
    # train kd with fewer samples
    logger.info(f"Teacher model trained with {data_args.max_train_samples} samples")
    data_args.max_train_samples = data_args.max_train_samples // 4
    logger.info(f"Training student model with {data_args.max_train_samples} samples")

    ## Prepare separate train dataset for student model
    if training_args.do_train:
        if "train" not in raw_datasets:
            raise ValueError("--do_train requires a train dataset")
        kd_train_dataset = raw_datasets["train"]
        if data_args.max_train_samples is not None:
            # randomly select a subset of the training data
            max_train_samples = min(
                len(kd_train_dataset), data_args.max_train_samples)
            all_indices = np.arange(len(kd_train_dataset))
            filtered_indices = np.setdiff1d(all_indices, teacher_train_indices)
            indices = np.random.choice(
                filtered_indices, size=max_train_samples, replace=False)
            kd_train_dataset = kd_train_dataset.select(indices)

    # tokenize and encode datasets
    with training_args.main_process_first(desc="dataset map pre-processing"):
        if training_args.do_train:
            kd_train_dataset = kd_train_dataset.map(
                preprocess_function,
                batched=True,
                batch_size=1000,
    ...
    ### End block -- get kd training dataset ###
    ...
    ### EDIT HERE ### CHANGES FROM 20240426 --START
    t_model_dir = "/llmft/model/teacher_model_3"
    t_model_dir_tk = "/llmft/model/teacher_model_3/tokens"
    ### CHANGE train_t_model to false if we are loading a pre-trained teacher model from t_model_dir
    train_t_model = True

    
    # Comment out this block when doing Kd training on student model -- start --

    # CHANGES 20240426: Added condition to train teacher model if train_t_model is True
    if train_t_model:
        ...
        if training_args.do_train:
            ...
            logger.info("======Fine Tuning training metrics======")
            ...
            # CHANGES 20240426: Save model to t_model_dir
            # Save model - run once to save model. then comment out and reload as t_model.
            print("Saving model...\n")
            trainer.save_model(t_model_dir)

            if trainer.is_world_process_zero():
                tokenizer.save_pretrained(t_model_dir_tk)
                trainer.save_state()

        ## Knowledge distillation part
        # Getting this error: 8 pytorch allocator cache flushes since last step. this happens when there is high memory pressure and is detrimental to performance. if this is happening frequently consider adjusting settings to reduce memory consumption.
        # Delete the trainer to free up memory to try fix this error
        del trainer
        torch.cuda.empty_cache()

        #indent
        # Uncomment out this block when doing Kd training on student model -- start --
    if training_args.do_train:
        ## Refer to config definition    
        # Reload the model
        print("training_args.do_train... \n config: ", config)

        ### CHANGES 20240426: Added condition to load teacher model if train_t_model is False
        if train_t_model == False:
            ...
            t_model = OPTWithLMClassifier(config)

            t_state_dict = torch.load("{}/pytorch_model.bin".format(t_model_dir))
            t_model.load_state_dict(t_state_dict)
            t_model.to(dtype=torch.float16, device=training_args.device)
            print("t_model loaded... \n")

        # Load student model
        if ft_args.target_tokens is not None:
            if train_t_model:
                ### CHANGES 20240426: Use teacher model from model object if we are using trained model directly
                s_model = OPTWithLMClassifierKD.from_pretrained_with_teacher(
                ...
                ### CHANGES 20240426: Use teacher model from t_model if we are reloading t_model and not training it 
                s_model = OPTWithLMClassifierKD.from_pretrained_with_teacher(
                    pretrained_model_name_or_path=model_args.model_name_or_path,
                    from_tf=bool(".ckpt" in model_args.model_name_or_path),
        ...
        print("s_model.config:", s_model.config)
        print("s_model.device:", s_model.device)

        # set model configuration (copied from above)
        # Some models have set the order of the labels to use, so let's make sure we do use it.
        label_to_id = None
        if (
            s_model.config.label2id != PretrainedConfig(
                num_labels=num_labels).label2id
            and data_args.task_name is not None
            and not is_regression
        ):
            # Some have all caps in their config, some don't.
            label_name_to_id = {
                k.lower(): v for k, v in s_model.config.label2id.items()}
            if list(sorted(label_name_to_id.keys())) == list(sorted(label_list)):
                label_to_id = {
                    i: int(label_name_to_id[label_list[i]]) for i in range(num_labels)}
            else:
        ...
        if label_to_id is not None:
            s_model.config.label2id = label_to_id
            s_model.config.id2label = {
                id: label for label, id in config.label2id.items()}
        elif data_args.task_name is not None and not is_regression:
            s_model.config.label2id = {l: i for i, l in enumerate(label_list)}
            s_model.config.id2label = {
                id: label for label, id in config.label2id.items()}
        ...
        print("Modifying training_args.warmup_steps.... ")
        # Initialize trainer for student
        #training_args.warmup_steps = data_args.max_train_samples // 4  # seems to fix the error. don't know why.
        print("training_args.warmup_steps", training_args.warmup_steps)
        print("Loading KD trainer... \n")
        print("training_args.half_precision_backend:", training_args.half_precision_backend)
        #training_args.half_precision_backend = "auto"
        print("training_args.half_precision_backend:", training_args.half_precision_backend) 
        print("training_args.fp16:", training_args.fp16)
        #training_args.fp16 = False
        print("training_args.fp16:", training_args.fp16)

        kd_trainer = FtTrainer(
            model=s_model,
            args=training_args,
        ...
        kd_train_result = kd_trainer.train(resume_from_checkpoint=checkpoint, ignore_keys_for_eval=["past_key_values"])

        logger.info("======KD training metrics======")
        metrics = kd_train_result.metrics
        max_train_samples = (
            data_args.max_train_samples if data_args.max_train_samples is not None else len(
                kd_train_dataset)
        )
        metrics["train_samples"] = min(max_train_samples, len(train_dataset))
        kd_trainer.log_metrics("train", metrics)
        kd_trainer.save_metrics("train", metrics)
    ...
    if training_args.push_to_hub:
        kd_trainer.push_to_hub(**kwargs)
    else:
        kd_trainer.create_model_card(**kwargs)
